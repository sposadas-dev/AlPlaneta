...
public class Reporte {
    ...
    public void reporteCancelacionReserva(Pagos_PasajeDTO pago){
    	Collection<Pagos_PasajeDTO> collection = new ArrayList<Pagos_PasajeDTO>();
		collection.add(pago);
    	//Hardcodeado
		Map<String, Object> parametersMap = new HashMap<String, Object>();
		parametersMap.put("Fecha", new SimpleDateFormat("dd/MM/yyyy").format(new Date()));		
    	try	{
			this.reporte = (JasperReport) JRLoader.loadObjectFromFile( "reportes" + File.separator + "ReporteCancelacioReserva.jasper" );
			this.reporteLleno = JasperFillManager.fillReport(this.reporte, parametersMap, 
					new JRBeanCollectionDataSource(collection));
    		log.info("Se cargó correctamente el reporte");
		}
